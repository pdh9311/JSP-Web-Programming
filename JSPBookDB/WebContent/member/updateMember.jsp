...
<sql:query dataSource="${dataSource}" var="resultSet">
	select * from jspbook_member where id=?
	...
	<div class="container">
		<form name="newMember" class="form-horizontal"	action="processUpdateMember.jsp" method="post" onsubmit="return checkForm()">
			...
			<div class="form-group row">
				...
				<div class="col-sm-3">
					<input type="password" name="pw" class="form-control" placeholder="password" value="${row.pw}" />
				</div>
			</div>
			<div class="form-group row">
			...
			<div class="form-group row">
				...
				<div class="col-sm-10">
					<input type="radio" name="gender" value="남" <c:if test="${row.gender.equals('남')}"><c:out value="checked" /></c:if>/> 남
					<input type="radio" name="gender" value="여" <c:if test="${row.gender.equals('여')}"><c:out value="checked" /></c:if>/> 여
				</div>
			</div>
			<div class="form-group row">
				<label class="col-sm-2">생일</label>
				<div class="col-sm-3">
					<input type="text" name="bithyy" maxlength="4" placeholder="년(4자)" size="6" value="${birthyy}"/>
					<select name="bithmm" id ="birthmm">
					...
					<input type="text" name="bithdd" maxlength="2" placeholder="일" size="4" value="${birthdd}"/>
				</div>
			</div>
			<div class="form-group row">
